...
- M1 connector v1 (`connector/Code.gs`, `connector/appsscript.json`):
  ...
  - If the web app has no active sheet, `doGet` opens the sheet ID that the menu stored. That fallback needs the wider scopes in spike step T9.
...
- Unit tests for the connector (through `vm`), template sync, fetcher classifier, URL validation and load-test statistics (including the first request's latency).
